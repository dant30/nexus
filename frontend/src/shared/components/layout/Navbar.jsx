...
export function Navbar({ onMenuClick, sidebarOpen }) {
  ...
  return (
    <>
      {/* Backdrop Blur - Appears when ANY dropdown is open */}
      {(openUserMenu || openAccountMenu) && (
        <div
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="relative" ref={accountMenuRef}>
                ...
                {openAccountMenu && accounts && accounts.length > 0 && (
                  <div className="absolute right-0 top-full z-50 mt-2 w-72 max-w-[calc(100vw-1rem)] overflow-hidden rounded-xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl shadow-black/40 ring-1 ring-black/50">
                    ...
                    <div className="max-h-56 overflow-y-auto bg-slate-900/50">
                      {accounts.map((account) => (
                        <button
                          ...
                          className={`flex w-full items-center justify-between gap-2 bg-transparent px-4 py-3 text-sm transition-colors ${
              ...
              <div className="relative" ref={userMenuRef}>
                ...
                {openUserMenu && (
                  <div className="absolute right-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl shadow-black/40 ring-1 ring-black/50">
                    <div className="border-b border-white/10 bg-slate-900/50 px-4 py-3">
                      <p className="text-xs font-semibold text-white truncate">{displayName}</p>
                      <p className="text-xs text-white/40 truncate">{user?.email || user?.username}</p>
                    </div>

                    <div className="space-y-1 bg-slate-900/50 p-2">
                      <Link
                        to="/settings/profile"
                        onClick={() => setOpenUserMenu(false)}
                        className="flex items-center gap-3 rounded-lg bg-transparent px-3 py-2.5 text-sm text-white/80 transition-colors hover:bg-white/5 hover:text-white"
                      >
                        <User size={16} />
                        <span>Profile</span>
                      </Link>
                    ...
                    <div className="border-t border-white/10 bg-slate-900/50 p-2">
                      <button
                        ...
                        className="flex w-full items-center gap-3 rounded-lg bg-transparent px-3 py-2.5 text-sm text-danger/80 transition-colors hover:bg-danger/10 hover:text-danger"
                      >
                        <LogOut size={16} />
                        <span>Sign out</span>
                      </button>
