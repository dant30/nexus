    ...
    <Card className="group relative min-h-[184px] overflow-hidden">
      ...
      <div className="relative flex items-start justify-between">
        <div className="min-w-0 flex-1">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.12em] text-white/50">Account Balance</p>
          <p className="truncate text-2xl font-bold text-white sm:text-3xl">
            {loading ? "--" : formatMoney(balance, currency)}
          </p>
          <p className={`mt-1 text-xs font-semibold uppercase tracking-[0.12em] ${typeClass}`}>{typeLabel}</p>
        </div>
