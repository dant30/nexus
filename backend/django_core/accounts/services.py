...
def create_demo_account(user, currency: str = "USD", initial_balance: Decimal = Decimal("10000.00")) -> Account:
    acc = Account.objects.create(
        user=user,
        account_type=Account.TYPE_DEMO,
        currency=currency,
...
def create_or_update_real_account(user, account_id: str, currency: str, metadata: dict, is_default: bool = False) -> Account:
    acc, _ = Account.objects.update_or_create(
        user=user, account_id=account_id,
        defaults={
            "account_type": Account.TYPE_REAL,
            "currency": currency,
